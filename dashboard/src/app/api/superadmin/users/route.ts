...
// GET /api/superadmin/users?tenantId=... - List users in a specific tenant
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching tenant users for superadmin:', error);
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 });
  }
}

// POST /api/superadmin/users - Create/invite user in a specific tenant
export async function POST(request: NextRequest) {
  try {
    ...
    const { tenantId, username, email, fullName, role } = body;

    // Validate request inputs
    if (!tenantId || !username || !email || !fullName || !role) {
    ...
    console.error('Error creating tenant user for superadmin:', error);
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 });
  }
}
